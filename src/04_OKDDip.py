...
def lookup(model_name):
    ...
    #     input_channel = 256
    elif model_name == "wrn_20_8" or model_name == "wrn_28_10":
        input_channel = 256
    # imagenet
...
def test(args, net, testloader):
    ...
    with torch.no_grad():
        ...
        for data in testloader:
            ...
            output_batch, x_m, x_stu = net(images)
            for i in range(args.num_branches - 1):
                _, predicted = torch.max(output_batch[:, :, i].data, 1)
                correct[i] += (predicted == labels).sum()
            
            _, predicted = torch.max(x_stu.data, 1)
            correct[args.num_branches - 1] += (predicted == labels).sum()
...
def train(args, nets, train_loader, opts, consistency_weight, criterion, criterion_T):
    ...
    for i, (imgs, labels) in enumerate(tqdm(train_loader)):
        ...
        output_batch, x_m, x_stu  = nets[0](imgs)
        loss_true = 0
        loss_group = 0    
        for i in range(args.num_branches - 1):
        ...
        loss = loss_true + criterion(x_stu, labels) + \
            args.alpha * consistency_weight * (loss_group + \
                criterion_T(x_stu, torch.mean(output_batch, dim = 2)))
...
def main():
    ...
    for name in args.model_names:
        if 'resnet' in name or 'wrn' in name:
            model = model_dict[name](num_classes=args.num_classes, \
                num_branches = args.num_branches, \
                input_channel = lookup(name.strip('_odkkip'))).to(args.device)
        else:
            model = model_dict[name](num_classes=args.num_classes, \
    ...
    # spath = '/nfs3-p1/zhangtl/01-Project/03_Ensemble/OKDPH/ckpt/cifar100/resnet32_okddip/net0_126ep.pkl'
    # nets[0].load_state_dict(torch.load(spath, map_location=args.device))
...
if __name__ == "__main__":
    ...
    # 2. Model & Loss
    parser.add_argument("--model_names", type=str, nargs='+', default=['resnet32_okddip']) # densenetd40k12_okddip
