...
def train(args, nets, train_loader, opts):
    ...
    for i, (imgs, labels) in enumerate(tqdm(train_loader)):
        ...
        stable_out = torch.stack(out_list).mean(dim=0).detach() # [n, b, num_classes] - > []

        for net_idx, net in enumerate(nets):
            ce_loss = F.cross_entropy(out_list[net_idx], labels)
            div_loss = F.kl_div(
...
if __name__ == "__main__":
    ...
    if len(args.transes) == 0:
        if 'cifar100' in args.dataset:
            args.transes = ['hflip+rot_seed']*len(args.model_names)
        elif 'cifar10' in args.dataset:
            args.transes = ['hflip_seed']*len(args.model_names)
